...
const Grid: React.FunctionComponent<{
  ...
  let bestSolution = Math.max(
    ...grid.map((row) => Math.max(...row.map((cell) => cell.value)))
  );

  const getBestSOlution = () => {
    const allBestSolutionsClicked = grid.every((row) =>
      row
        .filter((cell) => cell.value === bestSolution)
      ...
      {grid.map((row) =>
        row.map((cell) => (
          <Cell
            ...
            cell={cell}
            className={
              helpMode && cell.value === getBestSOlution()
